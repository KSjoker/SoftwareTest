    ...
    public class Dungeon
    {
        ...
        int addMonster(int zone, int toAdd, int added)
        {
            ...
            foreach (Node node in zones[zone])
            {
                ...
                if (GlobalRNGSwitch.GlobalRNG.RNGSwitch)
                {
                    newPack = new Pack(MonsterID(), node, 10, 10, 1, toAdd + 1); //Create new Pack with a random number of monsters
                }
                else
                {
                    newPack = new Pack(MonsterID(), node, 10, 10, 1); //Create new Pack with 1 monster
                }
